...
**Project images show as grey placeholders locally.**
File upload support is built into the admin panel (the project edit form has a cover image field and a gallery repeater). However, local file storage without a CDN or cloud bucket is not suitable for a portfolio. In production, you would configure an S3-compatible service (Cloudflare R2, AWS S3, etc.) by updating `FILESYSTEM_DISK` in the environment file. The infrastructure is already wired — it just needs a production storage driver.
